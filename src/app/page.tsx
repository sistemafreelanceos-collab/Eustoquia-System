...
import { LayoutGrid, Zap, FolderOpen, MousePointer2, Type, Image as ImageIcon, Link as LinkIcon, Mic, FileText, Music, Play } from 'lucide-react';
...
export default function Home() {
  ...
  const handleSelectBoard = (id: string) => {
    setActiveBoardId(id);
    fetchBoard(id); // Carga los nodos de este tablero
  };
    ...
    <div className="h-screen w-screen bg-[#08080D] flex flex-col font-sans overflow-hidden">
      ...
      <header className="h-[52px] bg-[#111118] border-b border-[#252535] flex items-center px-4 gap-3 shrink-0 z-50">
        <div className="flex items-center gap-2.5">
          <div className="w-[30px] h-[30px] rounded-max bg-gradient-to-br from-[#6C5CE7] to-[#FF6B9D] flex items-center justify-center text-white font-space font-extrabold text-[15px] rounded-lg">
            E
          </div>
          <span className="text-[#E4E4EE] font-space font-bold text-sm tracking-tight">
            Eustoquia System
          </span>
        </div>
        ...
        <div className="px-3 py-1.5 rounded-lg bg-[#6C5CE7]/15 text-[#6C5CE7] text-xs font-semibold">
          📋 Producción Semanal — @vendecomopro
        </div>

        <div className="flex-1" />

        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#F5A623]/10 border border-[#F5A623]/20">
          <Zap size={14} className="text-[#F5A623]" fill="currentColor" />
          <span className="text-[#F5A623] text-[11px] font-mono font-bold">847</span>
          <span className="text-[#555568] text-[9px] font-mono">créditos</span>
        </div>

        <div className="flex items-center -space-x-2">
          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#6C5CE7] to-[#00D2FF] border-2 border-[#08080D] flex items-center justify-center text-[11px] font-bold text-white">O</div>
          <div className="w-7 h-7 rounded-full bg-gradient-to-br from-[#2D0A4E] to-[#A29BFE] border-2 border-[#08080D] flex items-center justify-center text-[11px] font-bold text-white">G</div>
        </div>

        <button 
          onClick={() => setShowTemplates(!showTemplates)}
          className={`px-3.5 py-1.5 rounded-lg text-xs font-semibold transition-all ${showTemplates ? 'bg-[#6C5CE7] text-white' : 'bg-transparent border border-[#252535] text-[#9999B0]'}`}
        >
          🗂 Plantillas
        </button>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[60px] bg-[#111118] border-r border-[#252535] flex flex-col items-center py-3 gap-1 shrink-0 z-40">
          <ToolbarButton icon={<LayoutGrid size={18} />} label="Grupo" active />
          <ToolbarButton icon={<Zap size={18} />} label="Chat IA" active />
          <div className="w-8 h-[1px] bg-[#252535] my-1.5" />
          <ToolbarButton icon={<Play size={16} />} label="YouTube" />
          <ToolbarButton icon={<Mic size={16} />} label="Audio" />
          <ToolbarButton icon={<ImageIcon size={16} />} label="Imagen" />
          <ToolbarButton icon={<FileText size={16} />} label="Doc" />
          <ToolbarButton icon={<Type size={16} />} label="Nota" />
...
function ToolbarButton({ icon, label, active = false }: { icon: any, label: string, active?: boolean }) {
  return (
    <button className={`w-[42px] h-[42px] rounded-xl flex flex-col items-center justify-center gap-0.5 transition-all group ${active ? 'bg-[#6C5CE7]/12 border border-[#6C5CE7]/30 text-[#6C5CE7]' : 'text-[#555568] hover:bg-[#1A1A24] hover:text-[#9999B0]'}`}>
...
function BoardItem({ id, name, color, active = false, onRename, onClick }: { id?: string, name: string, color: string, active?: boolean, onRename?: (id: string, name: string) => void, onClick?: () => void }) {
  ...
  return (
    <div 
      ...
      className={`p-3 rounded-xl flex items-center gap-2.5 cursor-pointer transition-all ${active ? 'bg-[#6C5CE7]/10 border border-[#6C5CE7]/30' : 'hover:bg-[#1A1A24]'}`}
    >
      ...
      {isEditing ? (
        ...
        <span className={`text-xs ${active ? 'text-[#E4E4EE] font-semibold' : 'text-[#9999B0]'}`}>{name}</span>
      )}
    </div>
  );
}
